...
async def test_quantity_is_sum_of_transactions(client, admin_user, manager_user):
    product_id = await _create_product(client, admin_user)
...
async def test_stock_out_blocked_when_it_would_go_negative(client, admin_user, manager_user):
    product_id = await _create_product(client, admin_user)
...
async def test_stock_out_to_exactly_zero_is_allowed(client, admin_user, manager_user):
    product_id = await _create_product(client, admin_user)
...
async def test_employee_can_view_products_and_history(client, admin_user, employee_user):
    product_id = await _create_product(client, admin_user)
...
async def test_manager_can_create_and_record_stock(client, manager_user):
    product_id = await _create_product(client, manager_user, name="Manager Widget")
    resp = await client.post(
        f"/stock/{product_id}/transaction",
